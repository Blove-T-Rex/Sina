...
class Digital extends Component{
	...
	render(){
      ...
      >
					...
					{this.state.list.map((res,index)=>{
						...
						<div className="icon">
						<NavLink className="chuan" to={'/one/'+res.mblog.id} key={'/home/'+res.mblog.id}><Icon type="aliyun"  style={{fontSize:'20px'}}/><span  className="host">{res.mblog.reposts_count==null?'转发':res.mblog.reposts_count}</span></NavLink>
						<NavLink className="chuan" to={'/twe/'+res.mblog.id} key={'/factor/'+res.mblog.id}><Icon type="message" style={{fontSize:'20px'}}/><span className="host">{res.mblog.comments_count==null?'评论':res.mblog.comments_count}</span></NavLink>
						<NavLink className="chuan" to={'/three/'+res.mblog.id} key={'/hash/'+res.mblog.id}><Icon type="like-o" style={{fontSize:'20px'}}/><span className="host">{res.mblog.attitudes_count==null?'赞':res.mblog.attitudes_count}</span></NavLink>
	...
	componentDidMount(){
		...
		axios.get("/api/container/getIndex?containerid=102803_ctg1_4188_-_ctg1_4188&openApp=0").then(res=>{
